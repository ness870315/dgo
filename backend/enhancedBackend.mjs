...
import { isExcludedToken } from './utils/excludedTokens.js';
    ...
    this.app.get('/api/tokens/trending/ai-analysis', async (req, res) => {
      ...
      try {
        ...
        if (!this.trendingAIService) {
          ...
          this.trendingAIService = new TrendingTokensAIAnalysisService();
        }
        
        // Generate AI analysis
        const analysisResult = await this.trendingAIService.analyzeTrendingTokens(limit);
        
        if (!analysisResult.success) {
          return res.status(500).json({ 
            error: 'analysis_failed', 
            message: analysisResult.error || 'Failed to analyze trending tokens' 
          });
        }
        
        // Return formatted response
        if (format === 'text') {
          const textReport = this.trendingAIService.formatAsTextReport(analysisResult);
    ...
    */ // END REMOVED PREDICTION ENDPOINTS
    ...
    this.app.get('/api/tokens/:contract/realtime-data', async (req, res) => {
      try {
        ...
        // Build response with swap history included
        const responseData = {
          ...realTimeData,
          recentSwaps: swapHistory,
          swapHistory: swapHistory,
    ...
    try {
      ...
      if (this.enhancedHybridPriceService) {
        ...
        this.enhancedHybridPriceService.initializeAsync().catch(error => {
          console.error('❌ Failed to start DEX stream:', error.message);
        });
      }
