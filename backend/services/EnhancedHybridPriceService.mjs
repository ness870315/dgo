...
class EnhancedHybridPriceService extends EventEmitter {
    ...
    async shutdown() {
        ...
        // Stop shared stream (the main stream for all tokens)
        if (this.sharedStream) {
            try {
                console.log('🔌 [EnhancedHybridPriceService] Closing shared stream...');
                this.sharedStream.removeAllListeners();
                this.sharedStream.cancel();
                this.sharedStream.end();
                this.sharedStream.destroy();
                this.sharedStream = null;
                console.log('✅ [EnhancedHybridPriceService] Shared stream closed');
            } catch (error) {
                console.error('❌ [EnhancedHybridPriceService] Error closing shared stream:', error.message);
            }
        }
        
        // Stop all individual gRPC streams (if any)
        if (this.grpcStreams && this.grpcStreams.size > 0) {
            console.log(`🔌 [EnhancedHybridPriceService] Closing ${this.grpcStreams.size} individual streams...`);
            this.grpcStreams.forEach((stream, key) => {
                try {
                    ...
                    console.error(`❌ [EnhancedHybridPriceService] Error closing stream ${key}:`, error.message);
                }
            });
            this.grpcStreams.clear();
            console.log('✅ [EnhancedHybridPriceService] All individual streams closed');
        }
        
        // Close gRPC client
        if (this.grpcClient) {
            try {
                console.log('🔌 [EnhancedHybridPriceService] Closing gRPC client...');
                this.grpcClient.close();
                this.grpcClient = null;
                console.log('✅ [EnhancedHybridPriceService] gRPC client closed');
            } catch (error) {
                console.error('❌ [EnhancedHybridPriceService] Error closing gRPC client:', error.message);
            }
        }
        ...
        // Stop ChartDatabase batch writer
        if (this.chartDatabase) {
            try {
                await this.chartDatabase.stopBatchWriter();
                console.log('✅ [EnhancedHybridPriceService] ChartDatabase batch writer stopped');
            } catch (error) {
                console.error('❌ [EnhancedHybridPriceService] Error stopping ChartDatabase:', error.message);
            }
        }
